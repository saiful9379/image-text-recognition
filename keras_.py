...
from ctc_data_generator_add_multi_noise import preprocess, get_image
...
alphabet = string.digits + 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ '
np.random.seed(55)
...
def add_7random_noise(image):
    
    def _speckle(img):
        # assume pixels range is in [0, 1]
        severity = np.random.uniform(0, 0.6)
        blur = ndimage.gaussian_filter(np.random.randn(*img.shape) * severity, 1)
        img_speck = (img + blur)
        img_speck[img_speck > 1] = 1
    ...
    noises = ['original_image', 'gaussian', 'localvar', 'poisson', 'salt', 'pepper', 's&p', 'speckle']
    # randomly select a noise
    noise_name = random.choice(noises)

    a = np.array(image, np.uint8) 
    a = a[:, :, 0]  # grab single channel
    # # print('shape after grab singl channel')
    a = a.astype(np.float32) / 255
    a = np.expand_dims(a, 0)

    if noise_name == 'original_image':
        return a
    elif noise_name == 'speckle':
        return _speckle(a)
    else:
        return sk.util.random_noise(a, mode=noise_name, clip=True)

cnt = 1
def paint_text(image_file_name, w, h, rotate=False, ud=False, multi_fonts=False):
    target_canvas = (w, h)
    a = get_image(image_file_name, target_canvas)
    # ////////////// add only speckle noise //////////////////
    ...
    # ///////////// add 7 different noise ////////////////////
    a = add_7random_noise(a)
    # ////////////////////////////////////////////////////////
    return a
...
class TextImageGenerator(keras.callbacks.Callback):
    ...
    def __init__(self, image_data_path, minibatch_size,
        ...
        self.all_images = os.listdir(image_data_path)
    ...
    def build_word_list(self, num_words, max_string_len=None):
        ...
        for image_file in self.all_images:
            img_abs_path = self.image_data_path + '/' + image_file
            if len(gt_text_and_fname_pair) == self.num_words:
                break
            text = image_file.split('.')[0].split('_')[-1]
            if is_valid_str(text) and _is_length_of_word_valid(text):
                # gt_texts.append(text)
                # X_text_image_file_name.append(image_file)
        ...
        if len(gt_text_and_fname_pair) != self.num_words:
            raise IOError('Could not pull enough words'
                          'from supplied image data directory.')
        ...
        # print('len X_text:', len(self.X_text))
    ...
    def get_batch(self, index, size, train):
        ...
        for i in range(size):
            ...
            #     # print(' i > size -4 index:', index, ' i', i, ' index + i:', index + i)
            #     # print('////////////////// I am on chennels frist ///////////////')
            #     if K.image_data_format() == 'channels_first':
            ...
            #     source_str.append('')
            # else:
            # print('index:', index, ' i', i, ' index + i:', index + i)
            # if K.image_data_format() == 'channels_first':
            ...
            # print(self.paint_func(self.X_text_image_file_name[index + i]))
            # print('ok')
            # print('file name index', index + i)
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        # if 3 <= epoch < 6:
        #     print('3 <= epoch < 6')
        #     self.build_word_list(16000, 8, 1)
        #     # self.paint_func = lambda text: paint_text(
        #     #     text, self.img_w, self.img_h,
        ...
        elif epoch >= 21 and epoch < 40: # added by me
            self.build_word_list(9280, 20)
...
def train(run_name, start_epoch, stop_epoch, img_w):
    # Input Parameters
    img_h = 64
    words_per_epoch = 9280# 3200 # 4000
    val_split = 0.1
    ...
    imgs_data_dir = 'path/to/the/image directory'
